...
def section_title(text: str):
    st.markdown(
        f'<div class="section-title">{text}</div>', unsafe_allow_html=True)


def info_box(text: str):
    st.markdown(
        f'<div class="info-box">ℹ️ {text}</div>', unsafe_allow_html=True)


def warn_box(text: str):
    st.markdown(
        f'<div class="warn-box">⚠️ {text}</div>', unsafe_allow_html=True)
...
def filter_sidebar(df: pd.DataFrame) -> dict:
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown(
        '<div class="nav-label">Filtros</div>', unsafe_allow_html=True)

    coberturas = ["Todas"] + sorted(df["COBERTURA"].dropna().unique().tolist())
    cobertura = st.sidebar.selectbox(
        "Cobertura", coberturas, key="sb_cobertura")
...
def footer():
    ...
    <div class="custom-footer">
      🏥 <b>Análise IGR</b> · Disciplina: Algoritmo II · Autor: <b>Jocimir Araujo</b><br>
